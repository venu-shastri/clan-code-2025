...
bool searchNamesStartingWithT(const std::string& name) {
    return !name.empty() && (name[0] == 't' || name[0] == 'T');
}
bool searchNamesEndsWithS(const std::string& name) {
    return !name.empty() && (name[0] == 's' || name[0] == 'S');
}
...
int main() {
    ...
    searchNames(names,searchNamesEndsWithS);
